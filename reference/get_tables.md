...
A family of functions that return a tibble from a specific database from
Hydroscope using the Enhydris API. `get_database` returns a named list
of tibbles using all the family's functions.
...
For example, time series' IDs from http://kyy.hydroscope.gr have same
values with time series' from http://ypaat.hydroscope.gr.
